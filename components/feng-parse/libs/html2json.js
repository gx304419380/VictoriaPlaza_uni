  ...
  HTMLParser(html, {
    start(tag, attrs, unary) {
      ...
      if (node.tag === 'img') {
        ...
        // 这里的逻辑主要是先展示缩略图，所以这样处理。但是没有考虑表情的情况。这里先加一层逻辑，如果 thumbUrl 没有图片的时候默认不处理
        // http://bug.eims.com.cn/bug-view-32.html
        if (attach && attach.thumbUrl) {
          imgUrl = attach.thumbUrl;
